...
class BBOBVisualizer(object):
    ...
    def __init__(
        ...
        X: chex.Array,
        fitness: chex.Array,
        true_grads: Optional[chex.Array] = None,
        est_grads: Optional[chex.Array] = None,
    ...
    def init(self):
        ...
        if self.use_3d:
            ...
            if self.est_grads is not None:
                self.est_arrow = self.ax.arrow(
                        self.X[0, :, 0].item(),
                        self.X[0, :, 1].item(),
                        self.est_grads[0,0].item(),
                        self.est_grads[0,1].item(),
                        width=0.1,
                        fc="r",
                        alpha=0.7,
                )
            if self.true_grads is not None:
                self.true_arrow = self.ax.arrow(
                        self.X[0, :, 0].item(),
                        self.X[0, :, 1].item(),
                        self.true_grads[0,0].item(),
                        self.true_grads[0,1].item(),
                        width=0.1,
                        fc="b",
                        alpha=0.7,                        
                )
    ...
    def update(self, frame):
        ...
        if self.est_grads is not None:
            self.est_arrow.set_data(
                x=self.X[frame, 0, 0],
                y=self.X[frame, 0, 1],
                dx=self.est_grads[frame, 0],
                dy=self.est_grads[frame, 1],
            )
        if self.true_grads is not None:
            self.true_arrow.set_data(
                x=self.X[frame, 0, 0],
                y=self.X[frame, 0, 1],
                dx=self.true_grads[frame, 0],
                dy=self.true_grads[frame, 1],
            )
        if self.use_3d:
            if self.plot_log_fn:
